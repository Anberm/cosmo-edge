...
TEST_CASE("Inference pipeline metrics expose host, graph, and RKNN stage timings", "[nn][rknn][metrics]") {
    ...
    metrics.RecordRknnRgaCropResize(850'000, true);
    metrics.RecordRknnRgaCropResize(50'000, false);
    metrics.RecordRknnRgaCropSource(true);
    metrics.RecordRknnRgaCropSource(false);
    metrics.RecordRknnRgaFailure();
    metrics.RecordRknnCpuResizeFallback(900'000);
    metrics.RecordRknnCpuCropResizeFallback(950'000);
    ...
    CHECK(snapshot.rknn_rga_crop_resize_calls == 2);
    CHECK(snapshot.rknn_rga_crop_resize_nanoseconds == 900'000);
    CHECK(snapshot.rknn_rga_crop_resize_failures == 1);
    CHECK(snapshot.rknn_rga_crop_dmabuf_frames == 1);
    CHECK(snapshot.rknn_rga_crop_host_fallbacks == 1);
    CHECK(snapshot.rknn_rga_failures == 1);
    CHECK(snapshot.rknn_cpu_resize_fallback_calls == 1);
    CHECK(snapshot.rknn_cpu_crop_resize_fallback_calls == 1);
    CHECK(snapshot.rknn_cpu_crop_resize_fallback_nanoseconds == 950'000);
