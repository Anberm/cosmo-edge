...
TEST_CASE("accelerator telemetry exposes additive load semantics", "[json][system][metrics]") {
    ...
    original.mppRgaCopyOutFrames                = 4;
    original.mppRgaCopyInFrames                 = 3;
    original.mppCpuCopyOutFallbacks             = 1;
    original.mppCpuCopyInFallbacks              = 2;
    original.rknnRgaCropResizeCalls             = 7;
    original.rknnRgaCropResizeMs                = 3.5;
    original.rknnRgaCropResizeFailures          = 1;
    original.rknnRgaCropDmaBufFrames            = 6;
    original.rknnRgaCropHostFallbacks           = 1;
    original.rknnCpuCropResizeFallbackCalls     = 2;
    original.rknnCpuCropResizeFallbackMs        = 4.5;
    ...
    CHECK(doc["mppRgaCopyOutFrames"] == 4);
    CHECK(doc["mppRgaCopyInFrames"] == 3);
    CHECK(doc["rknnRgaCropResizeCalls"] == 7);
    CHECK(doc["rknnRgaCropDmaBufFrames"] == 6);
    CHECK(doc["rknnRgaCropHostFallbacks"] == 1);
    CHECK(doc["rknnCpuCropResizeFallbackCalls"] == 2);
    ...
    CHECK(restored.mppRgaCopyOutFrames == original.mppRgaCopyOutFrames);
    CHECK(restored.mppRgaCopyInFrames == original.mppRgaCopyInFrames);
    CHECK(restored.mppCpuCopyOutFallbacks == original.mppCpuCopyOutFallbacks);
    CHECK(restored.mppCpuCopyInFallbacks == original.mppCpuCopyInFallbacks);
    CHECK(restored.rknnRgaCropResizeCalls == original.rknnRgaCropResizeCalls);
    CHECK(restored.rknnRgaCropResizeMs == original.rknnRgaCropResizeMs);
    CHECK(restored.rknnRgaCropResizeFailures == original.rknnRgaCropResizeFailures);
    CHECK(restored.rknnRgaCropDmaBufFrames == original.rknnRgaCropDmaBufFrames);
    CHECK(restored.rknnRgaCropHostFallbacks == original.rknnRgaCropHostFallbacks);
    CHECK(restored.rknnCpuCropResizeFallbackCalls == original.rknnCpuCropResizeFallbackCalls);
    CHECK(restored.rknnCpuCropResizeFallbackMs == original.rknnCpuCropResizeFallbackMs);
