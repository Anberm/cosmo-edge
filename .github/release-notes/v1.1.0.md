...
- Multi-platform ScenarioBench material for controlled single workloads,
  concurrent mixed workloads, fixed-profile 72-hour observations, and short-run
...
observed discard. The VLM comparison publishes display references of 6 channels
on BM1688, 6 on CV186X, and 4 on RK3576. See the
...
- Real RTSP network interruption and recovery are outside the controlled
  local-loop test.
- VLM figures come from 60-second short-run steps using a uniform 80% display
  reference.
