...
for (const platform of platforms) {
  expectedReports.push(`results/${platform}/report.html`);
  for (const workload of workloads) {
    expectedReports.push(`results/${platform}/${workload}/report.html`);
  }
}
...
for (const main of ['report.html', 'report.zh-CN.html']) {
  ...
  for (const platform of platforms) {
    for (const workload of workloads) {
      const target = `results/${platform}/${workload}/report.html`;
...
console.log(`Benchmark page smoke test passed: ${expectedReports.length} rendered reports and 18 main-report attachment links.`);
